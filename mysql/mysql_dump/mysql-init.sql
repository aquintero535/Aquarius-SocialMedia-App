...
CREATE TABLE `users_follows`(
    ...
    FOREIGN KEY(profile_id_one) REFERENCES users_profiles(profile_id)
        ON UPDATE CASCADE ON DELETE CASCADE  
);
...
CREATE TABLE `posts`(
    ...
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `reply_to` CHAR(16),
    FOREIGN KEY(profile_id) REFERENCES users_profiles(profile_id)
        ON UPDATE CASCADE ON DELETE CASCADE 
);
...
CREATE TABLE `posts_likes`(
    ...
    FOREIGN KEY(profile_id) REFERENCES users_profiles(profile_id) ON UPDATE CASCADE ON DELETE CASCADE,
